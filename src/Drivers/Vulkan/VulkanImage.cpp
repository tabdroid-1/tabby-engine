...
void VulkanImage::CreateTexture()
{
    ...
    VulkanShaderBuffer staging_buffer(staging_buffer_spec, m_Specification.pixels.data(), m_Specification.pixels.size());
    ...
    for (int i = 0; i < copy_regions.size(); i++) {
        UIntVector2 mip_size = { m_Specification.extent.x / (uint32_t)(std::pow(2, i)), m_Specification.extent.y / (uint32_t)(std::pow(2, i)) };
...
//     render_target_create_info.format = convert(m_Specification.format);
//     render_target_create_info.samples = VK_SAMPLE_COUNT_1_BIT; // HACK
//     // render_target_create_info.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT;
//     render_target_create_info.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
//     render_target_create_info.tiling = VK_IMAGE_TILING_OPTIMAL;
//     render_target_create_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
//
//     auto allocator = VulkanMemoryAllocator::Get();
