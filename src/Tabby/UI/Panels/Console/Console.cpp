...
static void bool_setter(bool& var, bool active)
{
    var = active;
}
...
csys::ItemLog& operator<<(csys::ItemLog& log, glm::vec<L, T, Q>& vec)
{
...
csys::ItemLog& operator<<(csys::ItemLog& log, glm::mat<C, R, T, Q>& mat)
{
...
csys::ItemLog& operator<<(csys::ItemLog& log, glm::qua<T, Q>& qua)
{
    log << glm::to_string(qua);
    return log;
}
...
bool InputText(const char* label, std::string* str, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    ImGui::InputTextCallback_UserData cb_user_data;
    cb_user_data.Str = str;
    cb_user_data.ChainCallback = callback;
    cb_user_data.ChainCallbackUserData = user_data;
    return InputText(label, (char*)str->c_str(), str->capacity() + 1, flags, ImGui::InputTextCallback, &cb_user_data);
}

ConsolePanel::ConsolePanel(std::string c_name, size_t inputBufferSize)
...
csys::System& ConsolePanel::System()
{
    return m_ConsoleSystem;
}

void ConsolePanel::InitIniSettings()
{
...
void ConsolePanel::DefaultSettings()
{
    // Settings
    m_AutoScroll = true;
...
void ConsolePanel::RegisterConsoleCommands()
{
    m_ConsoleSystem.RegisterVariable("app_vsync", Tabby::Application::GetSpecification().VSync, bool_setter);
    m_ConsoleSystem.RegisterVariable("app_max_fpx", Tabby::Application::GetSpecification().MaxFPS, float_setter);
    uint8_t temp_uint8 = 0;
    m_ConsoleSystem.RegisterVariable("window_fullscreen_mode", temp_uint8, window_fullscreen_setter);
    ...
    m_ConsoleSystem.RegisterCommand("filter", "Set screen filter", [this](const csys::String& filter) {
        ...
        // Build text filter.
        m_TextFilter.Build(); }, csys::Arg<csys::String>("filter_str"));

    m_ConsoleSystem.RegisterCommand("run", "Run given script", [this](const csys::String& filter) {
        // Logs command.
        m_ConsoleSystem.RunScript(filter.m_String); }, csys::Arg<csys::String>("script_name"));
}

void ConsolePanel::FilterBar()
{
    m_TextFilter.Draw("Filter", ImGui::GetWindowWidth() * 0.25f);
    ImGui::Separator();
}

void ConsolePanel::LogWindow()
{
    ...
    if (ImGui::BeginChild("ScrollRegion##", ImVec2(0, -footerHeightToReserve), false, 0)) {
        ...
        for (const auto& item : m_ConsoleSystem.Items()) {
            ...
            if (item.m_Type == csys::COMMAND) {
                if (m_TimeStamps)
                    ImGui::PushTextWrapPos(ImGui::GetColumnWidth() - timestamp_width); // Wrap before timestamps start.
                if (count++ != 0)
                    ImGui::Dummy(ImVec2(-1, ImGui::GetFontSize())); // No space for the first command.
            }
...
void ConsolePanel::InputBar()
{
    ...
    if (InputText("Input", &m_Buffer, inputTextFlags, InputCallback, this)) {
        // Validate.
        if (!m_Buffer.empty()) {
...
// From imgui_demo.cpp
void ConsolePanel::HelpMaker(const char* desc)
{
    ImGui::TextDisabled("(?)");
...
int ConsolePanel::InputCallback(ImGuiInputTextCallbackData* data)
{
    ...
    std::string trim_str;
    auto console = static_cast<ConsolePanel*>(data->UserData);
...
void ConsolePanel::SettingsHandler_ClearALl(ImGuiContext* ctx, ImGuiSettingsHandler* handler)
{
}

void ConsolePanel::SettingsHandler_ReadInit(ImGuiContext* ctx, ImGuiSettingsHandler* handler)
{
}

void* ConsolePanel::SettingsHandler_ReadOpen(ImGuiContext* ctx, ImGuiSettingsHandler* handler, const char* name)
{
    if (!handler->UserData)
        return nullptr;

    auto console = static_cast<ConsolePanel*>(handler->UserData);
...
void ConsolePanel::SettingsHandler_ReadLine(ImGuiContext* ctx, ImGuiSettingsHandler* handler, void* entry, const char* line)
{
    if (!handler->UserData)
        return;
...
void ConsolePanel::SettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    if (!handler->UserData)
        return;
