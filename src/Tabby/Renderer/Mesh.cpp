    ...
    : m_Specification(spec)
{
    ...
    buffer_spec.size = m_Specification.vertex_data.Size;

    m_VertexBuffer = ShaderBuffer::Create(buffer_spec, m_Specification.vertex_data);

    buffer_spec.buffer_usage = ShaderBufferUsage::INDEX_BUFFER;
    buffer_spec.heap = ShaderBufferMemoryHeap::DEVICE;
    buffer_spec.memory_usage = ShaderBufferMemoryUsage::NO_HOST_ACCESS;
    buffer_spec.size = m_Specification.index_data.Size;
    buffer_spec.flags |= (uint64_t)ShaderBufferFlags::INDEX_TYPE_UINT16;

    m_IndexBuffer = ShaderBuffer::Create(buffer_spec, m_Specification.index_data);

    if (spec.index_type == MeshIndexType::UINT8)
        m_TotalIndexCount = spec.index_data.Size / sizeof(uint8_t);
    else if (spec.index_type == MeshIndexType::UINT16)
        m_TotalIndexCount = spec.index_data.Size / sizeof(uint16_t);
    else if (spec.index_type == MeshIndexType::UINT32)
        m_TotalIndexCount = spec.index_data.Size / sizeof(uint32_t);

    if (!m_Specification.material)
        return;

    if (m_Specification.vertex_data.Size % m_Specification.material->GetShader()->GetVertexBufferSize() != 0) {
...
void Mesh::Destroy()
{
    m_Specification.vertex_data.Release();
    m_Specification.index_data.Release();
...
void Mesh::SetMaterial(Shared<Material> material)
{
    ...
    if (m_Specification.vertex_data.Size % material->GetShader()->GetVertexBufferSize() != 0) {
        TB_CORE_WARN("Vertex buffer size of shader that is from of material being set to mesh \"{}\", is not divisible by the mesh's vertex buffer size. Vertex data might be not suitable for the shader.", m_Specification.name);
    }
}
