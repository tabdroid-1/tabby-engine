...
Canvas::Canvas(Shared<Framebuffer> framebuffer, Vector2& viewportSize, std::array<Vector2, 2>& viewportBounds)
...
void Canvas::ImGuiRender()
{
    ...
    if (m_ShowGameView) {
        ImGui::Checkbox("Show Schematic", &m_ShowGameView);
        ...
        ImGui::Checkbox("Show game view", &m_ShowGameView);
        ...
        if (opt_enable_grid) {
            for (float x = fmodf(scrolling.x, pixelPerMeter); x < canvas_sz.x; x += pixelPerMeter)
                draw_list->AddLine(ImVec2(canvas_p0.x + x, canvas_p0.y), ImVec2(canvas_p0.x + x, canvas_p1.y), IM_COL32(200, 200, 200, 40));
            for (float y = fmodf(scrolling.y, pixelPerMeter); y < canvas_sz.y; y += pixelPerMeter)
