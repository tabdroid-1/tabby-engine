...
{
    return (p_node->op == SpvOpSpecConstant || p_node->op == SpvOpSpecConstantOp || p_node->op == SpvOpSpecConstantTrue || p_node->op == SpvOpSpecConstantFalse);
}
        ...
        case SpvOpAccessChain: {
            ...
            if (p_access_chain->index_count > 0) {
                ...
                for (uint32_t index_index = 0; index_index < p_access_chain->index_count; ++index_index) {
                    ...
                    if ((p_index_value_node != NULL) && (p_index_value_node->op == SpvOpConstant || p_index_value_node->op == SpvOpSpecConstant)) {
                        // Read index value
    ...
    size_t first_label_index)
{
    ...
    if (p_func->accessed_variable_count > 0) {
        p_func->accessed_variables = (SpvReflectPrvAccessedVariable*)calloc(p_func->accessed_variable_count, sizeof(*(p_func->accessed_variables)));
...
static SpvReflectResult ParseDecorations(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
    ...
    for (uint32_t i = 0; i < p_parser->node_count; ++i) {
        ...
        if ((p_node->op != SpvOpDecorate) && (p_node->op != SpvOpMemberDecorate) && (p_node->op != SpvOpDecorateId) && (p_node->op != SpvOpDecorateString) && (p_node->op != SpvOpMemberDecorateString)) {
            continue;
        }
            ...
            if (result == SPV_REFLECT_RESULT_SUCCESS) {
                ...
                if (IsNotNull(p_length_node)) {
                    ...
                    if (result == SPV_REFLECT_RESULT_SUCCESS) {
                        ...
                        p_type->traits.array.spec_constant_op_ids[dim_index] = IsSpecConstant(p_length_node) ? p_length_node->decorations.spec_id : (uint32_t)INVALID_VALUE;
                    } else {
                        result = SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE;
                        SPV_REFLECT_ASSERT(false);
                    }
    ...
    SpvReflectTypeDescription* p_type, SpvReflectBlockVariable* p_var)
{
    ...
    if (IsNotNull(p_type->members) && (p_type->member_count > 0)) {
        ...
        for (uint32_t member_index = 0; member_index < p_type->member_count; ++member_index) {
            ...
            if ((p_member_type->storage_class == SpvStorageClassPhysicalStorageBuffer) && (p_member_type->type_flags & SPV_REFLECT_TYPE_FLAG_REF)) {
    ...
    SpvReflectBlockVariable* p_var)
{
    ...
    for (uint32_t member_index = 0; member_index < p_var->member_count; ++member_index) {
        ...
        if (is_parent_root) {
            ...
            p_member_var->absolute_offset = is_parent_aos ? 0 : (is_parent_ref ? p_member_var->offset : p_member_var->offset + p_var->absolute_offset);
        }
    }
    ...
    SpvReflectBlockVariable** pp_member_offset_order = (SpvReflectBlockVariable**)calloc(p_var->member_count, sizeof(SpvReflectBlockVariable*));
    ...
    p_last_member_var->padded_size = RoundUp(p_last_member_var->offset + p_last_member_var->size, SPIRV_DATA_ALIGNMENT) - p_last_member_var->offset;
    ...
    SpvOp override_op_type, SpvReflectBlockVariable* p_var)
{
    ...
    case SpvOpTypeArray: {
        ...
        if (index_index < p_access_chain->index_count) {
            ...
            SpvReflectResult result = ParseDescriptorBlockVariableUsage(p_parser, p_module, p_access_chain, index_index, p_type->op, p_var);
            if (result != SPV_REFLECT_RESULT_SUCCESS) {
                return result;
            }
        } else {
    ...
    case SpvOpTypeStruct: {
        ...
        //
        if (index_index < p_access_chain->index_count) {
            SpvReflectResult result = ParseDescriptorBlockVariableUsage(p_parser, p_module, p_access_chain, index_index, (SpvOp)INVALID_VALUE, p_member_var);
            if (result != SPV_REFLECT_RESULT_SUCCESS) {
                return result;
            }
    ...
    uint32_t* p_interface_variable_ids)
{
    ...
    for (size_t i = 0; i < interface_variable_count; ++i) {
        ...
        SpvReflectResult result = ParseInterfaceVariable(p_parser, &p_node->decorations, &p_type_node->decorations, p_module, p_type, p_var, &has_built_in);
        if (result != SPV_REFLECT_RESULT_SUCCESS) {
            SPV_REFLECT_ASSERT(false);
            return result;
        }
...
{
    return IsNotNull(p_node) && IsNotNull(p_binding) && p_node->op == SpvOpAccessChain && p_node->word_count == 6 && (p_binding->user_type == SPV_REFLECT_USER_TYPE_BYTE_ADDRESS_BUFFER || p_binding->user_type == SPV_REFLECT_USER_TYPE_RW_BYTE_ADDRESS_BUFFER);
}
    ...
    for (size_t i = 0; i < p_func->accessed_variable_count; ++i) {
        if (p_func->parameters[function_parameter_index] == p_func->accessed_variables[i].variable_ptr) {
            ...
            if (p_var->function_id > 0) {
                SpvReflectResult result = ParseFunctionParameterAccess(p_parser, p_var->function_id, p_var->function_parameter_index, p_accessed);
                if (result != SPV_REFLECT_RESULT_SUCCESS) {
                    return result;
                }
            } else {
    ...
    for (uint32_t i = 0; i < p_module->descriptor_binding_count; ++i) {
        ...
        for (uint32_t j = 0; j < used_acessed_count; j++) {
            ...
            if (p_var->variable_ptr == p_binding->spirv_id) {
                if (p_var->function_id > 0) {
                    result = ParseFunctionParameterAccess(p_parser, p_var->function_id, p_var->function_parameter_index, &p_binding->accessed);
                    if (result != SPV_REFLECT_RESULT_SUCCESS) {
                        SafeFree(p_used_accesses);
                        return result;
                    }
        ...
        if (byte_address_buffer_offset_count > 0) {
            ...
            if (multi_entrypoint) {
                ...
                p_binding->byte_address_buffer_offsets = (uint32_t*)calloc(byte_address_buffer_offset_count + p_binding->byte_address_buffer_offset_count, sizeof(uint32_t));
            ...
            if (multi_entrypoint) {
                ...
                p_binding->byte_address_buffer_offset_count = (uint32_t)DedupSortedUint32(p_binding->byte_address_buffer_offsets, p_binding->byte_address_buffer_offset_count);
            }
        }
    }
    ...
    if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
        ...
        for (size_t entry_point_idx = 0; entry_point_idx < p_module->entry_point_count; ++entry_point_idx) {
            ...
            if (p_entry_point->execution_mode_count > 0) {
                p_entry_point->execution_modes = (SpvExecutionMode*)calloc(p_entry_point->execution_mode_count, sizeof(*p_entry_point->execution_modes));
...
static SpvReflectResult ParsePushConstantBlocks(SpvReflectPrvParser* p_parser, SpvReflectShaderModule* p_module)
{
    ...
    for (size_t i = 0; i < p_parser->node_count; ++i) {
        ...
        for (uint32_t access_chain_index = 0; access_chain_index < p_parser->access_chain_count; ++access_chain_index) {
            ...
            SpvReflectBlockVariable* p_var = (p_access_chain->base_id == p_push_constant->spirv_id) ? p_push_constant : GetRefBlkVar(p_parser, p_access_chain);
