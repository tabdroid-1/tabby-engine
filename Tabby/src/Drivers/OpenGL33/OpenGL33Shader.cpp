...
OpenGL33Shader::OpenGL33Shader(const std::string& name, const std::string& vertexSrc, const std::string& fragmentSrc)
    : m_Name(name)
{
    TB_PROFILE_FUNCTION();
...
std::unordered_map<GLenum, std::string> OpenGL33Shader::PreProcess(const std::string& source)
{
    ...
    while (pos != std::string::npos) {
        ...
        TB_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
...
void OpenGL33Shader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
{
    TB_PROFILE_FUNCTION();
