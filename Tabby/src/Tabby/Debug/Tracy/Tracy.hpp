...
#define ZoneNamed(varname, active)                                                                                                                            \
    ...
    tracy::ScopedZone varname(&TracyConcat(__tracy_source_location, TracyLine), TRACY_CALLSTACK, active)
...
#define TracyLockable(type, varname)                   \
    tracy::Lockable<type> varname                      \
    {                                                  \
        []() -> const tracy::SourceLocationData* { static constexpr tracy::SourceLocationData srcloc { nullptr, #type " " #varname, TracyFile, TracyLine, 0 }; return &srcloc; }() \
    }
#define TracyLockableN(type, varname, desc)            \
    tracy::Lockable<type> varname                      \
    {                                                  \
        []() -> const tracy::SourceLocationData* { static constexpr tracy::SourceLocationData srcloc { nullptr, desc, TracyFile, TracyLine, 0 }; return &srcloc; }() \
    }
#define TracySharedLockable(type, varname)             \
    tracy::SharedLockable<type> varname                \
    {                                                  \
        []() -> const tracy::SourceLocationData* { static constexpr tracy::SourceLocationData srcloc { nullptr, #type " " #varname, TracyFile, TracyLine, 0 }; return &srcloc; }() \
    }
#define TracySharedLockableN(type, varname, desc)      \
    tracy::SharedLockable<type> varname                \
    {                                                  \
        []() -> const tracy::SourceLocationData* { static constexpr tracy::SourceLocationData srcloc { nullptr, desc, TracyFile, TracyLine, 0 }; return &srcloc; }() \
    }
...
#define LockMark(varname)                                                                                                                     \
    static constexpr tracy::SourceLocationData __tracy_lock_location_##varname { nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, 0 }; \
    varname.Mark(&__tracy_lock_location_##varname)
...
#define ZoneNamedS(varname, depth, active)                                                                                                                    \
    static constexpr tracy::SourceLocationData TracyConcat(__tracy_source_location, TracyLine) { nullptr, TracyFunction, TracyFile, (uint32_t)TracyLine, 0 }; \
    tracy::ScopedZone varname(&TracyConcat(__tracy_source_location, TracyLine), depth, active)
