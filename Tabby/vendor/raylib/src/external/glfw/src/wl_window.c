    ...
    int width, int height)
{
    ...
    decoration->subsurface = wl_subcompositor_get_subsurface(_glfw.wl.subcompositor,
    ...
    struct wl_output* output)
{
    ...
    if (window->wl.monitorsCount + 1 > window->wl.monitorsSize) {
        ...
        window->wl.monitors = _glfw_realloc(window->wl.monitors,
    ...
    uint32_t size)
{
    ...
    composeTable = xkb_compose_table_new_from_locale(_glfw.wl.xkb.context, locale,
...
static void dataDeviceHandleDataOffer(void* userData,
    ...
    struct wl_data_offer* offer)
{
    _GLFWofferWayland* offers = _glfw_realloc(_glfw.wl.offers, _glfw.wl.offerCount + 1);
    if (!offers) {
        _glfwInputError(GLFW_OUT_OF_MEMORY, NULL);
        return;
    }
...
{
    if (scancode < 0 || scancode > 255 || _glfw.wl.keycodes[scancode] == GLFW_KEY_UNKNOWN) {
...
void _glfwSetCursorWayland(_GLFWwindow* window, _GLFWcursor* cursor)
{
    ...
    if (window->cursorMode == GLFW_CURSOR_NORMAL || window->cursorMode == GLFW_CURSOR_CAPTURED) {
        if (cursor)
            ...
            struct wl_cursor* defaultCursor = wl_cursor_theme_get_cursor(_glfw.wl.cursorTheme, "left_ptr");
            if (!defaultCursor) {
                _glfwInputError(GLFW_PLATFORM_ERROR,
    ...
    } else if (window->cursorMode == GLFW_CURSOR_HIDDEN || window->cursorMode == GLFW_CURSOR_DISABLED) {
...
const char* _glfwGetClipboardStringWayland(void)
{
    ...
    _glfw_free(_glfw.wl.clipboardString);
    _glfw.wl.clipboardString = readDataOfferAsString(_glfw.wl.selectionOffer, "text/plain;charset=utf-8");
    return _glfw.wl.clipboardString;
}
